...
AuthorizeHandler.prototype.getUser = function(request, response) {
  ...
  if(request.session.userId) {
     return Promise.try(this.model.getSessionUser, request).then(function(user) {
       if (!user) {
          throw new ServerError('Server error: `model.getSessionUser` did not return a `user` object');
        }
        return user;
      });
  }
